...
public class EventHandler {
	...
	public EventHandler(GamePanel gp) {
		...
		eventRect = new EventRect[gp.maxWorldCol][gp.maxWorldRow]; // Inicializa a matriz de eventos com base no tamanho do mundo
		
		int col = 0;
		int row = 0;
		while(col < gp.maxWorldCol && row < gp.maxWorldRow) {
			
			eventRect[col][row] = new EventRect();   // Cria uma nova área de evento para cada tile
			eventRect[col][row].x = 23;				 // Define posição X relativa dentro do tile
			eventRect[col][row].y = 23; 			 // Define posição Y relativa dentro do tile
			eventRect[col][row].width = 2; 			 // Largura da área de detecção
			eventRect[col][row].height = 2; 		 // Altura da área de detecção
			eventRect[col][row].eventRectDefaultX = eventRect[col][row].x; // Armazena valor original para reset
